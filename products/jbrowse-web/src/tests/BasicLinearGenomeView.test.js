...
describe('valid file tests', () => {
  ...
  it('click and drag overview rubber band', async () => {
    const pluginManager = getPluginManager()
    const state = pluginManager.rootModel
    const { findByTestId } = render(<JBrowse pluginManager={pluginManager} />)
    ...
    expect(state.session.views[0].bpPerPx).toEqual(0.05)
    fireEvent.mouseDown(overviewRubberBand, { clientX: 100, clientY: 0 })
    fireEvent.mouseMove(overviewRubberBand, { clientX: 250, clientY: 0 })
    fireEvent.mouseUp(overviewRubberBand, { clientX: 250, clientY: 0 })
